//api.js
var added_at=0;async function post(e,r){error(!1);var o=document.cookie.replace(/(?:(?:^|.*;\s*)session\s*\=\s*([^;]*).*$)|^.*$/,"$1");o&&(e.session=o),e.redirect=!!r;var t=e=>{if("object"==typeof e)try{return JSON.stringify(e)}catch{}return e};if(!r){var n=new FormData;for(var i in e)n.append(i,t(e[i]));return fetch("/api/",{method:"POST",body:n}).then(e=>e.json()).catch(e=>{console.log({error:e.message}),"NetworkError when attempting to fetch resource."==e.message&&error("Network Disconnected")}).then(r=>{if(200!=r.code&&error(r.msg),""!=r.rethrow)return r;post(e,!0)})}var a=nu("form",{id:"api-form",action:"/api/",method:"POST"});for(var i in e)nu("input",{type:"hidden",name:i,value:t(e[i])},a);nu("input",{type:"submit",style:"visibility: hidden;"},[a,document.body],!0).click(),nu("api-form").remove()}function error(e){e?(added_at=Date.now(),nu("error").style.display="block",nu("error").innerText=e):Date.now()>added_at+5e3&&(nu("error").style.display="none",nu("error").innerText="")}
