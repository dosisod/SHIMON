...
//settings.js
function setting(e){var r=e.attributes["data-type"];r&&(r=r.value);var t=e.innerText.toLowerCase()||e.id;if("change password"==t){var o=prompt("Enter old password");if(!o)return;var n=prompt("Enter new password");if(!n)return;post({"change pwd":{old:o,new:n}})}else if("generate new key"==t){if(!confirm("Are you sure? Generating new key will make your friends unable to talk to you until your new public key is sent out"))return;if(!(i=prompt("Enter password to confirm")))return;post({"new key":i},!0)}else if("EXPIRATION TIMER"==r)post({"expiration timer":e.value});else if("MSG DELETION"==r)post({"msg policy":e.value});else if("THEME"==r)post({theme:e.value});else if("devmode"==t)post({devmode:e.checked}).then(e=>{window.location.reload(!0)});else if("nuke cache"==t){if(!confirm("Are you sure you want to delete cache?"))return;var i;if(!(i=prompt("Enter password to confirm")))return;post({nuke:i},!0)}}
