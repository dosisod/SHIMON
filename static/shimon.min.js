...
//settings.js
function setting(e){var r=e.innerText.toLowerCase()||e.id;if("change password"==r){var o=prompt("Enter old password");if(!o)return;var n=prompt("Enter new password");if(!n)return;post({"change pwd":{old:o,new:n}})}else if("generate new key"==r){if(!confirm("Are you sure? Generating new key will make your friends unable to talk to you until your new public key is sent out"))return;if(!(t=prompt("Enter password to confirm")))return;post({"new key":t},!0)}else if(["15 mins","1 hour","5 hours","1 day"].indexOf(r)>-1)post({"expiration timer":e.value});else if(["ask first","require password","dont ask"].indexOf(r)>-1)post({"msg policy":e.value});else if("darkmode"==r)post({darkmode:e.checked}).then(e=>{window.location.reload(!0)});else if("devmode"==r)post({devmode:e.checked}).then(e=>{window.location.reload(!0)});else if("nuke cache"==r){if(!confirm("Are you sure you want to delete cache?"))return;var t;if(!(t=prompt("Enter password to confirm")))return;post({nuke:t},!0)}}

//ui.js
var tray=document.getElementById("tray"),friends=[];async function check_friends(){friends.length||(friends=(friends=await post({data:"friends"})).msg)}function realname(e){for(var n of friends)if(n.id==e)return n.name}function uname(e){for(var n of friends)if(n.name==e)return n.id}async function reload_msgs(){await check_friends();var e=document.cookie.replace(/(?:(?:^|.*;\s*)uname\s*\=\s*([^;]*).*$)|^.*$/,"$1");if(preload){n=preload;preload=!1}else{var n;n=(n=await post({data:{allfor:e}})).msg}if(0!=n.length){var a=n.id,i=n.msgs;replace_template(new_card(n.hash,realname(e),"",!0,!0),e=>{var n=nu("span",{className:e.sending?"x-sending":"x-receiving",innerText:"x",onclick:n=>{confirm("Are you sure you want to delete this message?")&&(post({"delete msg":{id:a,index:e.index}}),reload_msgs())}},nu("li",{className:"item"}));return nu("span",{className:"msg",innerText:e.msg},[nu("div",{className:"holder block "+(e.sending?"sending":"receiving")}),n]),n},i,nu("span",{className:"center name point",innerText:"RELOAD",id:"reload",onclick:()=>reload_msgs()}))}}async function reload_index(){if(await check_friends(),preload){e=preload;preload=!1}else{var e;e=(e=await post({data:"recent"})).msg}replace_template(void 0,e=>new_card(e.hash,realname(e.id),e.msgs[e.msgs.length-1].msg,!0,!1,!0),e,nu("span",{className:"center name point",innerText:"RELOAD",onclick:()=>reload_index()}))}async function replace_template(e,n,a,i){tray.innerHTML='<div class="rightbar"><a class="rightitem name point" href="/add">ADD FRIEND</a><br><a class="rightitem name point" href="/account">ACCOUNT</a><br><span class="rightitem name point" onclick="save(event)">SAVE</span></div>',e&&tray.appendChild(e),a.forEach((e,a)=>{e.index=a,tray.appendChild(n(e))}),i&&tray.appendChild(i)}function new_card(e,n,a,i,r,t){var s=nu("ol");s.appendChild(nu("li",{className:"name hide",innerText:n})),s.appendChild(nu("li",{className:"msg hide",innerText:a}));var d=nu("div",{className:t?"holder point":"holder"});r||(d.onclick=()=>window.location="/msg/"+uname(n)),d.appendChild(nu("img",{src:new_img(e)})),d.appendChild(document.createTextNode("\n")),d.appendChild(s);var l=nu("li",{className:"item"});if(l.appendChild(d),i)return l;tray.appendChild(l)}function new_img(e){var n=nu("canvas",{width:16,height:16}),a=n.getContext("2d"),i="";for(var r of e){var t=parseInt(r,16).toString(2);i+="0".repeat(4-t.length)+t}for(r=0;r<256;r++)a.fillStyle="1"==i[r]?"rgb(255,255,255)":"rgb(0,0,0)",a.fillRect(r%16,~~(r/16),1,1);return n.toDataURL()}
