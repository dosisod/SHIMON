//api.js
async function post(e,r){nu("error").style.display="none",nu("error").innerText="";var n=document.cookie.replace(/(?:(?:^|.*;\s*)session\s*\=\s*([^;]*).*$)|^.*$/,"$1");n&&(e.session=n),e.redirect=!!r;var o=e=>{if("object"==typeof e)try{return JSON.stringify(e)}catch{}return e};if(!r){var t=new FormData;for(var i in e)t.append(i,o(e[i]));return fetch("/api/",{method:"POST",body:t}).then(e=>e.json()).catch(e=>{console.log({error:e.message}),"NetworkError when attempting to fetch resource."==e.message&&(nu("error").style.display="block",nu("error").innerText="Network Disconnected")}).then(r=>{if(200!=r.code&&(nu("error").style.display="block",nu("error").innerText=r.msg),""!=r.rethrow)return r;post(e,!0)})}var s=nu("form",{id:"api-form",action:"/api/",method:"POST"});for(var i in e)nu("input",{type:"hidden",name:i,value:o(e[i])},s);nu("input",{type:"submit",style:"visibility: hidden;"},[s,document.body],!0).click(),nu("api-form").remove()}

//lock.js
function lock(e){e.preventDefault();var o=prompt("Re-enter password to lock");o&&post({lock:o},!0)}function save(e){e.preventDefault();var o=prompt("Re-enter password to save");o&&post({save:o}).then(e=>{200==e.code&&(nu("error").style.display="block",nu("error").innerText="Cache was successfully saved")})}
