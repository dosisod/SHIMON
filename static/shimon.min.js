//api.js
async function post(e,t){document.getElementById("error").style.display="none",document.getElementById("error").innerText="";var r=document.cookie.replace(/(?:(?:^|.*;\s*)session\s*\=\s*([^;]*).*$)|^.*$/,"$1");r&&(e.session=r),e.redirect=!!t;var n=e=>{if("object"==typeof e)try{return JSON.stringify(e)}catch{}return e};if(!t){var o=new FormData;for(var i in e)o.append(i,n(e[i]));return fetch("/api/",{method:"POST",body:o}).then(e=>e.json()).catch(e=>{console.log({error:e.message}),"NetworkError when attempting to fetch resource."==e.message&&(document.getElementById("error").style.display="block",document.getElementById("error").innerText="Network Disconnected")}).then(t=>{if(200!=t.code&&(document.getElementById("error").style.display="block",document.getElementById("error").innerText=t.msg),""!=t.rethrow)return t;post(e,!0)})}var d=nu("form",{id:"api-form",action:"/api/",method:"POST"});for(var i in e)nu("input",{type:"hidden",name:i,value:n(e[i])},d);nu("input",{type:"submit",style:"visibility: hidden;"},[d,document.body],!0).click(),document.getElementById("api-form").remove()}
