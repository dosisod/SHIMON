...
class TestUnlock(BaseTest):
	...
	def test_incorrect_pwd_adds_to_attempts(self):
		self.unlock("not the password")

		assert self.shimon.login_limiter.attempts==1

	@BaseTest.request_context
	def test_valid_pwd_ignored_when_in_cooldown(self):
	...
	@BaseTest.request_context
	def test_correct_pwd_resets_login_limiter(self):
	...
	@BaseTest.request_context
	def test_mistached_versions_shows_warning_page(self):
