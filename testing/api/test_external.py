...
class TestApiFriends(BaseTest):
	...
	@BaseTest.unlocked
	def test_hash_is_not_added_to_cache(self):
...
class TestApiRecent(BaseTest):
	@BaseTest.request_context
	@BaseTest.unlocked
	def test_returned_data_is_valid(self):
...
class TestApiAllfor(BaseTest):
	...
	@BaseTest.request_context
	@BaseTest.unlocked
	def test_redraw_disabled_returns_empty_array(self):
	...
	@BaseTest.request_context
	@BaseTest.unlocked
	def test_redraw_is_disabled_after_running(self):
		self.shimon.redraw=True
		api_allfor(self.shimon, self.user["id"])
		assert self.shimon.redraw==False

class TestSha256hex(BaseTest):
	def test_bytes_and_strings_return_equal_data(self):
		assert sha256hex("123")==sha256hex(b"123")

	def test_sha256_against_known_hash(self):
