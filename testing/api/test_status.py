...
class TestStatus(BaseTest):
    ...
    def test_always_returns_http_200(self) -> None:
        assertHttpResponse(self.status(True), 200)

    @BaseTest.request_context
    def test_unlocked_false_when_locked(self) -> None:
        assert self.shimon.cache.is_empty() != self.status(True)[0].json["unlocked"]
    ...
    def test_that_redirect_flag_doesnt_change_output(self) -> None:
        assert (
            self.status(True)[0].json == json.loads(self.status(False)[0].data)["msg"]
        )
    ...
    def assertStatus(self, key: str) -> None:
        current_status = getattr(
            self.shimon, self.shimon.cache.mapper.cache_names[key]  # type: ignore
        )
