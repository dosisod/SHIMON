...
class TestSendMsg(BaseTest):
    ...
    def test_invalid_data_returns_http_400(self) -> None:
        assertHttpResponse(ApiSendMsg().entry(self.shimon, "not valid", False), 400)

    @BaseTest.request_context
    def test_missing_uname_param_returns_http_400(self) -> None:
        assertHttpResponse(
            ApiSendMsg().entry(self.shimon, {"msg": "hello"}, False), 400
        )
    ...
    def test_empty_msg_or_uname_returns_http_400(self) -> None:
        assertHttpResponse(
            ApiSendMsg().entry(self.shimon, {"uname": "whatever", "msg": None}, False),
            400,
        )

        assertHttpResponse(
            ApiSendMsg().entry(self.shimon, {"uname": None, "msg": "whatever"}, False),
            400,
        )
    ...
    def send_msg_wrapper(self, msg: str, uname: str = "") -> HttpResponse:
        ...
        return ApiSendMsg().entry(self.shimon, {"msg": msg, "uname": uname}, False)
